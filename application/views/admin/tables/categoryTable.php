    ...
    $i = 1;
      foreach ($categories as $r) { ?>
        <tr>
            ...
            <td>
                <?php if (@$r->is_active) { ?> 
                	<button type="button" class="btn btn-outline-success" onclick="offCat(<?= @$r->category_id ?>)">Online</button>
                <?php }else{ ?>
                	<button type="button" class="btn btn-outline-secondary" onclick="onCat(<?= @$r->category_id ?>)">Offline</button>
                <?php } ?>
            </td>
            <td class="text-center">
                ...
                <a href="javascript:void(0)" onclick="openModal('<?= base_url('admin/shiftProductForm/1/'.$r->category_id)?>','Product Shifting','md')"><span class="badge badge-success"><i class="icon-paperplane"></i> Product Shift</span></a>
            </td>
        </tr>
        <?php  } ?>
    </tbody>
